...
DEFAULT_REGISTRY = Path(__file__).resolve().parent.parent / "registry" / "candidates.json"


def load_registry(path: Path = DEFAULT_REGISTRY) -> CandidateRegistry:
    registry = CandidateRegistry.model_validate_json(path.read_text(encoding="utf-8"))
