...
def test_surface_scripts_compile_and_declare_persistent_master_contract() -> None:
    for relative in (
        "resources/adapters/bake_surface.py",
        "resources/adapters/prepare_semantic_surface_baseline.py",
        "resources/adapters/project_painted_vertex_surface.py",
        "resources/adapters/review_surface_master.py",
        "src/text2model_forge/surface_review.py",
    ):
        path = ROOT / relative
        compile(path.read_text(encoding="utf-8"), str(path), "exec")

    source = (ROOT / "resources/adapters/bake_surface.py").read_text(encoding="utf-8")
    assert "text2model_surface_master.blend" in source
...
def test_text2model_surface_runtime_config_keeps_paint_on_one_master(tmp_path: Path) -> None:
    namespace = runpy.run_path(str(ROOT / "resources/adapters/bake_surface.py"))
    master = tmp_path / "input.blend"
    baseline = tmp_path / "surface" / "semantic_surface_baseline.blend"
    baked = tmp_path / "surface" / "text2model_surface_master.blend"
    args = type(
        "Args",
        (),
        {
    ...
    assert config["id"] == "text2model_test_footman_surface"


def test_semantic_baseline_derives_generic_materials_from_asset_spec() -> None:
    source = (ROOT / "resources/adapters/prepare_semantic_surface_baseline.py").read_text(encoding="utf-8")
    ...
    assert "surface baseline requires Text2ModelClub" not in source


def test_qwen_surface_prompt_carries_before_current_numbers_and_stops_iteration() -> None:
    source = (ROOT / "src/text2model_forge/surface_review.py").read_text(encoding="utf-8")
