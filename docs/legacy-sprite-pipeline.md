...
VettedMesh turns owned Blender masters into deterministic sprites for a 2.5D battlefield. A character rig
...
## VettedMesh Studio quick start

The reusable product layer is game-neutral. Create a workspace and an asset card:

```powershell
.\scripts\setup_asset_forge.ps1
python -m assetforge init --workspace workspaces/my-game --project-id my_game --name "My Game"
python -m assetforge new --workspace workspaces/my-game --asset-id iron_guard --name "Iron Guard" --kind character --role "shield tank readable at phone size"
python -m assetforge prompt --workspace workspaces/my-game --asset-id iron_guard
python -m assetforge serve --workspace workspaces/my-game
```
...
python -m assetforge doctor
python -m assetforge register-model --workspace workspaces/my-game --model-id sdxl-base-1.0 --file runtime/comfyui/models/checkpoints/sd_xl_base_1.0.safetensors --reviewer <name>
```

Generate a provenance-tracked candidate:

```powershell
python -m assetforge generate --workspace workspaces/my-game --asset-id iron_guard --mode turnaround --seed 1001
```
...
```powershell
python -m assetforge approve --workspace workspaces/my-game --asset-id iron_guard --stage concept --artifact <selected.png> --reviewer <name>
.\scripts\setup_local_ai.ps1 install-triposr -AcceptTripoSrLicense
python -m assetforge mesh-seed --workspace workspaces/my-game --asset-id iron_guard --input <selected.png>
python -m assetforge audit-master --config <character-config.json> --report <master-audit.json>
.\scripts\asset_forge.ps1 render -CharacterConfig <character-config.json>
python -m assetforge qa-sheets --config <character-config.json> --report <sheet-qa.json> --contact-sheet <review.png>
```
...
The command builds a disposable master under `artifacts/vettedmesh-morphology/`,
...
hash-bound mesh report under `artifacts/vettedmesh-genesis/<id>/`, and verifies that
...
python -m assetforge register-model --workspace <ws> --model-id dreamshaper-xl-v2-turbo --file runtime/comfyui/models/checkpoints/dreamshaper_xl_v2_turbo.safetensors --reviewer <name>
python -m assetforge register-model --workspace <ws> --model-id controlnet-openpose-sdxl-xinsir --file runtime/comfyui/models/controlnet/controlnet_openpose_sdxl_xinsir.safetensors --reviewer <name>
python -m assetforge register-model --workspace <ws> --model-id controlnet-depth-sdxl-xinsir --file runtime/comfyui/models/controlnet/controlnet_depth_sdxl_xinsir.safetensors --reviewer <name>
python -m assetforge poses --workspace <ws> --preview          # editable pose pack + control-image previews
python -m assetforge animate --workspace <ws> --asset-id footman --actions idle,walk,attack,hit,death --reference <approved-concept.png> --seed 3001
python -m assetforge pack-sheets --workspace <ws> --asset-id footman   # prototype review only
```
...
After the owner visually accepts the generated board/GIF, bind approval to those exact bytes and publish from the
portable workspace:

```powershell
python -m assetforge approve --workspace <workspace> --asset-id example_biped --stage sheets --artifact <workspace>/assets/example_biped/reports/production-review/acceptance_board.png --reviewer <name>
...
Review outputs belong to the selected workspace and are deliberately not tied
to any consuming game repository.
...
`overpaint` block is enabled, Blender renders beauty, true depth, and equipment-protection passes. VettedMesh then:
...
output. Results go to `artifacts/vettedmesh-fixture/` and are not game art.
...
This generates `artifacts/vettedmesh-template/source/humanoid_template.blend` from repository-owned Python, then
...
named-action path as characters. Application UI assets are outside this renderer's contract.

## Source ownership

The required path is an original or properly licensed `.blend` master. No character pack, cloud API, or AI model is
required by the renderer. External rigging, animations, CC0 components, commissioned art, or image-to-3D may
...
See the repository README and `docs/darkness-worker-guide.md` for the current
product contract, stage gates, workspace layout, and adapter boundary.

## Art direction

The legacy renderer has no product-wide genre or named-style default. Put the
consuming project's palette, material language, silhouette rules, and camera in
that workspace's style profile. Use descriptive visual properties rather than
a living artist's name, a game title, or a protected character identity.
...
Style/medium: <the project's original material, shape, and rendering language>
Composition/framing: orthographic turnaround, neutral stance, equal scale, feet on one baseline
Lighting/mood: <the project's documented key light, contrast, and palette>
