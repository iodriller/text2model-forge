"""Darkness external-worker wrapper for MIT TRELLIS.2-4B.
...
model Darkness's RGBA-in policy (already applied to TripoSG and InstantMesh)
...
SOURCE_REVISION_ENV = "DARKNESS_TRELLIS2_SOURCE_REVISION"
...
class _NoNetworkRembg:
    ...
    def __call__(self, image):
        raise RuntimeError(
            ...
            "gated and non-commercial). Darkness never invokes it; this means the supplied "
...
def _dinov3_extract_features_compat(self, image):
    ...
    unpinned-dependency drift, not a Darkness policy decision; the fix
...
def _run(request: dict) -> tuple[Path, dict]:
    ...
    source = Path(os.environ["DARKNESS_TRELLIS2_SOURCE"]).resolve()
    ...
    weights = os.environ.get("DARKNESS_TRELLIS2_WEIGHTS", "microsoft/TRELLIS.2-4B")
