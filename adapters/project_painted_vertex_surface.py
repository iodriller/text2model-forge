...
def _ensure_uv(obj) -> None:
    mesh = obj.data
    existing = mesh.uv_layers.get("DarknessSurfaceUV")
    if existing is not None:
        mesh.uv_layers.remove(existing)
    mesh.uv_layers.new(name="DarknessSurfaceUV")
    mesh.uv_layers.active = mesh.uv_layers["DarknessSurfaceUV"]
...
def _project_object(obj, views) -> dict[str, float | int]:
    ...
    obj.data.materials.append(_paint_material(f"Darkness Projected {obj.name}"))
    obj["darkness_surface_projection"] = "comfyui_multiview_vertex_color_v1"
    return {
...
def main() -> int:
    ...
    armature = bpy.data.objects.get("DarknessShortBipedRig")
