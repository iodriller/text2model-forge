"""Darkness external-worker wrapper for MIT TripoSG without BRIA RMBG or DiSo."""
...
def _run(request: dict) -> tuple[Path, dict]:
    ...
    source = Path(os.environ["DARKNESS_TRIPOSG_SOURCE"]).resolve()
    weights = Path(os.environ["DARKNESS_TRIPOSG_WEIGHTS"]).resolve()
    compat = Path(os.environ["DARKNESS_TRIPOSG_COMPAT"]).resolve()
    ...
    if not _valid_alpha(image_path):
        raise ValueError(
            ...
            "Darkness will not fall back to non-commercial BRIA RMBG."
        )
