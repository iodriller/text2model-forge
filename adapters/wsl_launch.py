...
Bridges Darkness's native-Windows SubprocessWorkerAdapter contract
...
resulting response.json back to native Windows form, so the rest of Darkness
...
def windows_path_to_wsl(path: str) -> str:
    ...
    Only the `<drive>:\\...` / `<drive>:/...` form is supported; Darkness
...
def wsl_path_to_windows(path: str) -> str:
    ...
    own venv) that Darkness is not meant to dereference.
    """
...
def main(argv: list[str] | None = None) -> int:
    ...
    # owns WSL process trees directly (tracked in workers/trellis2.json).
