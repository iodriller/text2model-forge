...
ensure_ai_models() {
    ...
    if [[ "$selected_stack" == qwen ]]; then
        with_retry "Qwen Image 2512 model install" "$VENV_PYTHON" \
            "${REPO_ROOT}/adapters/install_qwen_image_edit_models.py" \
...
ensure_local_config() {
    ...
    canonical="${REPO_ROOT}/adapters/canonical_short_biped_worker.py"
    {
        ...
        if [[ -n "$BLENDER_BIN" ]]; then
            adapter="${REPO_ROOT}/adapters/blender_worker.py"
            printf '\n[workers.blender]\ncommand_prefix = [\n'
...
show_doctor() {
    ...
    if python_is_compatible "$VENV_PYTHON"; then
        ...
        "$VENV_PYTHON" -m darkness workers || true
    fi
}
...
run_smoke() {
    ...
    "$VENV_PYTHON" -m darkness demo --workspace "$smoke_root" --run-id launcher.demo.v1 || result=$?
    case "$smoke_root" in
