"""Resume the Text2Model motion-library -> evidence -> sprite -> Unity candidate chain."""
...
def _arguments(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--character-spec",
        type=Path,
        help="Text2Model Studio character_spec.json for equipment, naming, and surface semantics.",
    )
...
def _write_review_index(root: Path, *, unity_state: str) -> Path:
    ...
    path.write_text(
        "\n".join(
            [
                "# Text2Model painted motion candidate — human review",
                "",
                ...
                "- [Editable painted Blender master](surface/text2model_surface_master.blend)",
                "",
                "## Directional sprites",
                "",
...
def run(args: argparse.Namespace) -> dict[str, object]:
    ...
    status: dict[str, object] = {
        "schema_version": 1,
        "pipeline": "text2model_motion_surface_candidate_v2",
        "target": str(target),
    ...
    if existing.is_file():
        ...
        status = prior
        status["pipeline"] = "text2model_motion_surface_candidate_v2"
    ...
    if not retarget_glb.is_file():
        ...
        _record(status, root, "retarget", "running", "Baking four Quaternius actions onto the Text2Model rig.")
        retarget_command = [
                str(blender),
                "--background",
                "--factory-startup",
    ...
    surface_master = surface / "text2model_surface_master.blend"
    ...
    sprite_last = frames / "text2model_short_biped_candidate/death/west/09.png"
    ...
    if not _approved_automatic_candidate_manifest(
        ...
        _run(
            [
                ...
                str(frames),
                "--source-id",
                "text2model_short_biped_candidate",
                "--asset-id",
                asset_id,
                "--display-name",
                display_name,
    ...
    if not unity_report.is_file() and unity is not None and unity.is_file():
        ...
        environment["TEXT2MODEL_FORGE_CANDIDATE_PACKAGE"] = str(smoke_bundle / "candidate")
        environment["TEXT2MODEL_FORGE_CANDIDATE_OUTPUT"] = str(result_output)
        environment["TEXT2MODEL_FORGE_BUNDLE_MANIFEST"] = str(smoke_manifest)
        ...
        _run(
            [
                ...
                "-executeMethod",
                "Text2Model.EditorTools.Text2ModelCandidateValidator.ValidateFromBatch",
                "-logFile",
