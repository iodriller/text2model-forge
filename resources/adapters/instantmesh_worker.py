"""Text2Model external-worker wrapper for Apache-2.0 InstantMesh.
...
Text2Model always passes --no_rembg and a caller-owned pre-masked RGBA
...
SOURCE_REVISION_ENV = "TEXT2MODEL_FORGE_INSTANTMESH_SOURCE_REVISION"
...
def _run(request: dict) -> tuple[Path, dict]:
    ...
    source = Path(os.environ["TEXT2MODEL_FORGE_INSTANTMESH_SOURCE"]).resolve()
    ...
    if not _valid_alpha(image_path):
        raise ValueError(
            ...
            "Text2Model composites its own background and passes --no_rembg rather than trusting rembg/u2net."
        )
