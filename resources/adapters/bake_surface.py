"""Bake one ComfyUI-assisted painted master for every downstream Text2Model render."""
...
from text2model_forge.sprites.bake import _run_blender as run_bake_blender
from text2model_forge.sprites.bake import bake_texture_master
...
def _arguments(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        dest="asset_spec",
        type=Path,
        help="Text2Model Studio asset_spec.json used for material semantics and paint prompts.",
    )
...
def _config(
    ...
    work = output / "work"
    asset_id = str(spec.get("asset_id", "text2model_asset"))
    ...
    config = {
        "schema_version": 1,
        "id": f"text2model_{asset_id}_surface",
    ...
    if animated:
        config["animation_object"] = "Text2ModelShortBipedRig"
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if regenerate_baseline:
        ...
        for stale_anchor in (output / "anchors").glob("text2model_*_surface.anchor*.png"):
            stale_anchor.unlink()
    baked = output / "text2model_surface_master.blend"
    ...
    report = {
        "schema_version": 1,
        "stage": "text2model_surface_master",
