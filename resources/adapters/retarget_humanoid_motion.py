"""Retarget a small, declared humanoid motion set onto a Text2Model short-biped rig.
...
baked into new Text2Model-owned actions, and the imported source objects/actions
...
# Several source bones can feed one lower-resolution Text2Model bone.  This keeps
...
BONE_CHAINS = {
    ...
    "neck": ("neck_01",),
    "head": ("Head",),
    # The research Text2Model rig's historical suffixes describe its X side,
    ...
    # therefore maps to the Text2Model ``*_l`` chain.
...
EQUIPMENT = {
    ...
    "object_name": "Text2Model ForgeWeapon",
    "shield": None,
}
...
def _source_motion_hand_analysis(
    ...
    anatomical right hand is negative X, while the current research Text2Model
...
def _build_articulated_grip(
    ...
    armature.data.bones[str(EQUIPMENT["grip_socket_bone"])]["text2model_landmark_role"] = "grip_axis"
    for branch in branches:
        armature.data.bones[str(branch["base_bone"])]["text2model_landmark_role"] = "digit_base_to_joint"
        armature.data.bones[str(branch["tip_bone"])]["text2model_landmark_role"] = "digit_joint_to_tip"
    ...
    obj["text2model_grip_corrective"] = "articulated_digit_landmark_grip_v1"
    obj["text2model_grip_bone"] = bone_name
    obj["text2model_grip_affected_vertices"] = len(affected_indices)
    return {
...
def _create_club(armature: bpy.types.Object, body_height: float) -> bpy.types.Object:
    ...
    mesh = bpy.data.meshes.new("Text2ModelWeaponMesh" if is_sword else "Text2ModelClubMesh")
    geometry = bmesh.new()
    ...
    club["text2model_component_id"] = EQUIPMENT["component_id"]
    club["text2model_socket"] = EQUIPMENT["socket"]
    club["text2model_socket_bone"] = socket_bone
    club["text2model_rig_policy"] = EQUIPMENT["rig_policy"]
    club["text2model_equipment_category"] = "weapon"
    club["text2model_tip_reach_fraction"] = 0.55 if is_sword else 0.52
    constraint = club.constraints.new(type="COPY_TRANSFORMS")
    constraint.name = "Text2ModelHandSocket"
...
def _create_shield(armature: bpy.types.Object, body_height: float) -> bpy.types.Object | None:
    ...
    mesh = bpy.data.meshes.new("Text2ModelShieldMesh")
    geometry = bmesh.new()
    bmesh.ops.create_icosphere(
        geometry,
        subdivisions=2,
    ...
    geometry.free()
    mesh.update()
    shield = bpy.data.objects.new("Text2ModelShield", mesh)
    bpy.context.scene.collection.objects.link(shield)
    shield["text2model_component_id"] = str(shield_spec["equipment_id"])
    shield["text2model_socket"] = "forearm_left.shield"
    shield["text2model_socket_bone"] = socket_bone
    shield["text2model_rig_policy"] = "rigid_forearm_strap_v1"
    shield["text2model_equipment_category"] = "shield"
    constraint = shield.constraints.new(type="COPY_TRANSFORMS")
    constraint.name = "Text2ModelLeftForearmShieldSocket"
    constraint.target = armature
    constraint.subtarget = socket_bone
    constraint.target_space = "WORLD"
    constraint.owner_space = "WORLD"
...
def _validate_equipment_binding(
    ...
    *,
    ...
    tip_reach_fraction = float(club.get("text2model_tip_reach_fraction", 0.52))
...
def _validate_shield_binding(
    ...
    if shield is None:
        return None
    socket_bone = str(shield["text2model_socket_bone"])
    errors = []
    for clip_name, action in actions.items():
        armature.animation_data.action = action
        start, end = ranges[clip_name]
    ...
    return {
        "component_id": shield["text2model_component_id"],
        "mesh": shield.name,
        "socket": shield["text2model_socket"],
        "socket_bone": socket_bone,
        "rig_policy": shield["text2model_rig_policy"],
...
def main() -> int:
    ...
    target = _armature("Text2ModelShortBipedRig")
    ...
    for clip_name, spec in CLIPS.items():
        ...
        action["text2model_source"] = "Quaternius Universal Animation Library Standard"
