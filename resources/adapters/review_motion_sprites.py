...
from text2model_forge.sprite_review import LocalDeploySpriteReviewer  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.retarget_report:
        ...
        summary["equipment_history"] = [
            "empty-handed weapon-authored motion failed semantic review",
            "source and target sides are resolved from rest-space position",
            "the current candidate exposes its declared anatomical socket and grip metrics",
        ]
    ...
    review_path.write_text(
        "\n".join(
            [
                "# Text2Model sprite candidate review",
                "",
