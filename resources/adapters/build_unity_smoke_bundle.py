"""Build a portable, non-promoting Unity smoke project for a Text2Model sprite candidate."""
...
$env:TEXT2MODEL_FORGE_CANDIDATE_PACKAGE = $Package
$env:TEXT2MODEL_FORGE_CANDIDATE_OUTPUT = $Result
$env:TEXT2MODEL_FORGE_BUNDLE_MANIFEST = Join-Path $Root "bundle_manifest.json"

$Arguments = @(
    "-quit",
    "-batchmode",
    "-projectPath", ('"' + $Project + '"'),
    "-executeMethod", "Text2Model.EditorTools.Text2ModelCandidateValidator.ValidateFromBatch",
...
Write-Output "Text2Model Unity smoke validation passed. Return the result folder to the Text2Model pipeline."
'''
...
$Project = Join-Path $Root "UnitySmokeProject"
$env:TEXT2MODEL_FORGE_CANDIDATE_PACKAGE = Join-Path $Root "candidate"
$Arguments = @("-projectPath", ('"' + $Project + '"'))
Start-Process -FilePath $Unity -ArgumentList $Arguments
Write-Output "Unity review project launched. The Candidate Review window opens automatically; use Text2Model > Candidate Review if it was closed."
'''
...
<title>Text2Model Forge Candidate Motion Review</title>
<style>
...
<body>
<main>
  <h1>Text2Model Forge Candidate — Human Motion Review</h1>
...
def _readme_text() -> str:
    return f"""# Text2Model standalone Unity smoke check
...
- The Unity Candidate Review window opens automatically. Reopen it from `Text2Model > Candidate Review` if needed.
...
5. Resume the Text2Model pipeline; it will verify every returned hash before accepting the Unity proof.
...
def build(package: Path, output: Path, template: Path) -> dict[str, object]:
    ...
    bundle = {
        "schema_version": 1,
        "bundle_kind": "text2model_standalone_unity_smoke",
