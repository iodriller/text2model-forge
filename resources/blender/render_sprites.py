...
def add_sun(rotation, energy, color):
    light_data = bpy.data.lights.new(name="Text2ModelKey", type="AREA")
    light_data.energy = energy
    light_data.color = color
    light_data.shape = "DISK"
    light_data.size = 5.0
    light = bpy.data.objects.new(name="Text2ModelKey", object_data=light_data)
...
def configure_scene(config):
    ...
    if scene.render.use_freestyle:
        ...
        if line_set.linestyle is None:
            line_set.linestyle = bpy.data.linestyles.new("Text2Model Forge Outline")
    ...
    if scene.world is None:
        scene.world = bpy.data.worlds.new("Text2ModelWorld")
    scene.world.color = (0.018, 0.022, 0.03)

    camera_data = bpy.data.cameras.new("Text2ModelCamera")
    camera_data.type = "ORTHO"
    camera_data.ortho_scale = float(config.get("orthographic_scale", 4.6))
    camera = bpy.data.objects.new("Text2ModelCamera", camera_data)
    bpy.context.collection.objects.link(camera)
    scene.camera = camera

    add_sun((-4.0, -5.0, 8.0), 1400.0, (1.0, 0.86, 0.68))
    fill_data = bpy.data.lights.new(name="Text2ModelFill", type="AREA")
    fill_data.energy = 650.0
    fill_data.color = (0.24, 0.38, 0.60)
    fill_data.size = 6.0
    fill = bpy.data.objects.new(name="Text2ModelFill", object_data=fill_data)
    ...
    rim_data = bpy.data.lights.new(name="Text2ModelRim", type="AREA")
    rim_data.energy = 850.0
    rim_data.color = (0.55, 0.70, 0.95)
    rim_data.shape = "DISK"
    rim_data.size = 3.5
    rim = bpy.data.objects.new(name="Text2ModelRim", object_data=rim_data)
...
def build_depth_tree(scene, config):
    ...
    tree = bpy.data.node_groups.new("Text2ModelDepth", "CompositorNodeTree")
...
def render_equipment_mask(scene, equipment, mask_path):
    ...
    white = mask_material("Text2Model Mask White", (1.0, 1.0, 1.0))
    black = mask_material("Text2Model Mask Black", (0.0, 0.0, 0.0))
    slot_backups = []
    for obj in bpy.data.objects:
        if obj.type != "MESH" or obj.hide_render:
            continue
...
def main():
    ...
    print(f"TEXT2MODEL_FORGE_FRAMES={frames_root}")
