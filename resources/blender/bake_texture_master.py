...
def flat_scene(resolution):
    ...
    if scene.world is None:
        scene.world = bpy.data.worlds.new("Text2ModelBakeWorld")
    ...
    light_data = bpy.data.lights.new("Text2ModelBakeTop", type="AREA")
    light_data.energy = 420.0
    light_data.color = (1.0, 0.98, 0.95)
    light_data.shape = "DISK"
    light_data.size = 8.0
    light = bpy.data.objects.new("Text2ModelBakeTop", light_data)
    bpy.context.collection.objects.link(light)
    light.location = (0.0, 0.0, 8.0)

    camera_data = bpy.data.cameras.new("Text2ModelBakeCamera")
    camera_data.type = "ORTHO"
    camera = bpy.data.objects.new("Text2ModelBakeCamera", camera_data)
...
def build_depth_tree(distance):
    ...
    if is_group_api:
        tree = bpy.data.node_groups.new("Text2ModelBakeDepth", "CompositorNodeTree")
...
def render_views(args, config):
    ...
    print(f"TEXT2MODEL_FORGE_BAKE_VIEWS={views_folder}")
...
def ensure_bake_uv(obj):
    ...
    mesh = obj.data
    existing = mesh.uv_layers.get("Text2ModelBake")
    if existing is not None:
        mesh.uv_layers.remove(existing)
    layer = mesh.uv_layers.new(name="Text2ModelBake")
...
def project_bake(args, config):
    ...
    for obj in meshes:
        ...
        copy = posed_copy(obj)
        copy.data.uv_layers.active = copy.data.uv_layers["Text2ModelBake"]
        bpy.ops.object.select_all(action="DESELECT")
        ...
        material = build_projection_material(
            f"Text2ModelProjection_{obj.name}", view_records, fallback_image, target,
        ...
        baked = bpy.data.materials.new(f"Text2Model Baked {obj.name}")
    ...
    print(f"TEXT2MODEL_FORGE_BAKED_MASTER={destination}")
...
def render_icons(args, config):
    ...
    key_data = bpy.data.lights.new("Text2ModelIconKey", type="AREA")
    key_data.energy = 1000.0
    key_data.color = (1.0, 0.9, 0.75)
    key_data.size = 5.0
    key = bpy.data.objects.new("Text2ModelIconKey", key_data)
    ...
    print(f"TEXT2MODEL_FORGE_ICONS={folder}")
