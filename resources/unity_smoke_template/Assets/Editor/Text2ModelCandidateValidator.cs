...
{
    /// <summary>
    /// Validates an external Text2Model sprite candidate without copying it into the live
    ...
    /// </summary>
    public static class Text2ModelCandidateValidator
    {
        [Serializable]
        ...
        {
            var packagePath = Environment.GetEnvironmentVariable("TEXT2MODEL_FORGE_CANDIDATE_PACKAGE");
            var outputPath = Environment.GetEnvironmentVariable("TEXT2MODEL_FORGE_CANDIDATE_OUTPUT");
            var bundleManifestPath = Environment.GetEnvironmentVariable("TEXT2MODEL_FORGE_BUNDLE_MANIFEST");
            if (string.IsNullOrWhiteSpace(packagePath) || !Directory.Exists(packagePath))
            {
                throw new InvalidOperationException("TEXT2MODEL_FORGE_CANDIDATE_PACKAGE must name an existing package directory.");
            }

            if (string.IsNullOrWhiteSpace(outputPath))
            {
                throw new InvalidOperationException("TEXT2MODEL_FORGE_CANDIDATE_OUTPUT must name an evidence directory.");
            }

            if (string.IsNullOrWhiteSpace(bundleManifestPath) || !File.Exists(bundleManifestPath))
            {
                throw new InvalidOperationException("TEXT2MODEL_FORGE_BUNDLE_MANIFEST must name the immutable bundle manifest.");
            }
            ...
            try
            {
                ...
                Debug.Log("Text2Model candidate validation passed: " + JsonUtility.ToJson(report));
            }
            finally
            {
        ...
        private static void ValidateManifest(CandidateManifest manifest, string path)
        {
            ...
            {
                throw new InvalidOperationException("Invalid Text2Model candidate manifest: " + path);
            }
        ...
        {
            var root = new GameObject("Text2ModelCandidateCapture");
