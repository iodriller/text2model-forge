...
`darkness workers` already answers "does each worker exist". None of the
...
def check_donor_motion(stages: dict[str, Any], repo_root: Path) -> Check:
    ...
    if not clip_id:
        return Check(
            ...
            remedy="Set [stage_defaults.D7].donor_motion_id to a clip in motion_library/catalog.json.",
        )
    ...
    if not catalog.is_file():
        return Check(
            name="D7 donor motion",
            status="fail",
            detail=f"donor_motion_id={clip_id!r} but motion_library/catalog.json does not exist",
            remedy="Copy motion_library/catalog.example.json and fill in a real CC0 clip.",
        )
    try:
...
# What adapters/*.py actually assign to scene.render.engine, and the bone
# names adapters/retarget_humanoid_motion.py contracts against.
    ...
    if missing_bones or missing_actions:
        checks.append(
            Check(
                ...
                "adapters/prepare_donor_motion.py -- --source <in> --output <out>",
            )
        )
    else:
        checks.append(
