"""Headless access to Darkness Studio runs: list, show, and record gate
decisions without the browser control plane.

This shares StudioStore and StudioCoordinator with darkness.studio_web, so a
...
same limitation on the web side). Do not run `darkness studio decide` and
`darkness studio serve` against the same --workspace at the same time; a
