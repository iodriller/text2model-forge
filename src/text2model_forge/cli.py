"""Command-line entry point for local, resumable Text2Model runs."""
...
def _demo_brief() -> AssetBrief:
    return AssetBrief(
        asset_id="text2model-forge_fixture_demo",
        asset_type="character",
        title="Text2Model Forge Fixture Demo",
...
def run_demo(workspace: Path, run_id: str) -> dict:
    ...
    for target_stage in list(AssetStage)[1:]:
        ...
        artifact_id = f"text2model-forge_fixture_demo.{target_stage.name}.v1"
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="text2model-forge", description="Text2Model Forge asset compiler")
    ...
    demo.add_argument("--run-id", default="text2model_forge.demo.v1")
    ...
    # No subcommand (just `text2model_forge studio --workspace ... [--open-browser]`)
    ...
    studio_decide.add_argument(
        ...
        default=None,
        help="record that a person confirmed this current-attempt AI review recommendation",
    )
    studio_decide.add_argument(
        "--no-resume",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "studio":
        ...
        if studio_command == "decide":
            ...
            result = decide(
                ...
                assisted_by_review_id=args.assisted_by_review_id,
    ...
    if args.command == "glb-component-audit":
        ...
        print(json.dumps(result, indent=2, sort_keys=True))
        return 0
    compiler = Text2ModelCompiler(args.workspace, build_fake_registry())
