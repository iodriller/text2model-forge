...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="text2model-sprites",
        description="Text2Model Forge directional-sprite production CLI",
    )
    parser.add_argument("--version", action="version", version="Text2Model Forge Studio 0.2.0rc1")
    sub = parser.add_subparsers(dest="command", required=True)

    init = sub.add_parser("init", help="create a portable Text2Model Forge workspace")
    ...
    serve = sub.add_parser("serve", help="start the loopback-only Text2Model Forge Studio UI")
    ...
    if args.command == "create-unit":
        ...
        if reference is None:
            raise ForgeError(
                ...
                "(text2model-sprites generate --mode concept) and approve one (text2model-sprites approve --stage concept)"
            )
        if args.actions:
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        print(f"Text2Model Forge error: {error}", file=sys.stderr)
        return 2
    except KeyboardInterrupt:
        print("Text2Model Forge interrupted", file=sys.stderr)
        return 130
