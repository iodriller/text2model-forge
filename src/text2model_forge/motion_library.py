...
resources/adapters/retarget_humanoid_motion.py already retargets one donor animation
file onto a generated rig; this module is the addressable-by-name layer on
top of it that src/text2model_forge/settings.py's [stages.D7].donor_motion_id points at.
...
def resolve_donor_motion_path(library: MotionLibrary, clip_id: str, *, catalog_dir: Path) -> Path:
    ...
    file, ready to pass as --source to resources/adapters/retarget_humanoid_motion.py.
