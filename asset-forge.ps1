...
function Ensure-Core {
    ...
    $importWorks = $false
    try {
        & $script:VenvPython -c "import darkness, assetforge" 2>$null
    ...
    if ($SelectedStack -eq "qwen") {
        Invoke-WithRetry "Qwen Image 2512 model install" {
            Invoke-Native $script:VenvPython @(
                (Join-Path $script:RepoRoot "adapters\install_qwen_image_edit_models.py"),
...
function Ensure-LocalConfig {
    ...
    if ($Blender) {
        $adapter = Join-Path $script:RepoRoot "adapters\blender_worker.py"
        $lines.Add("")
    ...
    $canonical = Join-Path $script:RepoRoot "adapters\canonical_short_biped_worker.py"
    $lines.Add("")
...
function Show-Doctor {
    ...
    if (Test-PythonExecutable $script:VenvPython) {
        ...
        try {
            $workerJson = (& $script:VenvPython -m darkness workers) -join "`n"
...
try {
    ...
    try {
        Invoke-Native $script:VenvPython @("-m", "darkness", "demo", "--workspace", $smokeRoot, "--run-id", "launcher.demo.v1")
    }
    finally {
